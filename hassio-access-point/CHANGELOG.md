...
- Script cleanup and linting
- Converted logging to use `bashio`, and provide a wider range of logging levels
- Subnet config option now allows either CIDR or IP notation for the subnet mask
- Broadcast addresses are now optional, and left out by default for new configs
- Allowing internet access is now only from the AP, so as not to affect the host's networking
- Container files are now in the `rootfs` directory
